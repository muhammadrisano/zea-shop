import React from "react";

interface IcEditProps extends React.SVGProps<SVGSVGElement> {
    width?: string | number;
    height?: string | number;
    fill?: string;
}


const IcEdit:React.FC<IcEditProps> = ({ width = 16, height = 16, fill}) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={width}
      height={height}
      viewBox="0 0 16 16"
      fill="none"
    >
      <g clipPath="url(#clip0_1866_3226)">
        <path
          d="M0 12.6662V16H3.33379L13.1707 6.16302L9.8369 2.82922L0 12.6662Z"
          fill={fill || "#9B9B9B"}
        />
        <path
          d="M15.7399 2.33586L13.664 0.260036C13.3173 -0.0866786 12.7528 -0.0866786 12.4061 0.260036L10.7792 1.88693L14.113 5.22072L15.7399 3.59383C16.0866 3.24711 16.0866 2.68258 15.7399 2.33586Z"
          fill={fill || "#9B9B9B"}
        />
      </g>
      <defs>
        <clipPath id="clip0_1866_3226">
          <rect width="16" height="16" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};

export default IcEdit;
